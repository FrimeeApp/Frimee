...
export default function FlightsPage() {
  ...
  async function refreshFlights() {
    try {
      const data = await listActiveFriendFlightsEndpoint();
      setFlights(data);
  ...
  // Poll OpenSky via API route for each active flight every 30s.
  // The route also upserts to flight_positions → Realtime delivers updates to all other clients.
  ...
  async function pollFlight(ticket: PlanTicket) {
    ...
    if (isCapacitor) {
      // En Capacitor (WebView nativa) no hay CORS — llamamos OpenSky directo
      try {
      ...
      // En web usamos el proxy API route para evitar CORS
      try {
  ...
  async function pollAll() {
    for (const t of flightsRef.current) await pollFlight(t);
  }
  ...
  useEffect(() => {
    ...
    // Realtime: nuevos vuelos de amigos
    const flightsChannel = supabase
      .channel("active-flights")
      .on("postgres_changes", { event: "*", schema: "public", table: "plan_tickets" }, refreshFlights)
      .subscribe();

    // Realtime: posiciones escritas por cualquier cliente que haga polling
    const positionsChannel = supabase
      .channel("flight-positions")
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "flight_positions" },
        (payload) => {
          setPositions(prev => {
            const updated = { ...prev };
  ...
  // Start/restart polling whenever the flights list changes
  useEffect(() => {
    if (pollRef.current) clearInterval(pollRef.current);
    if (flights.length === 0) return;
    pollAll(); // immediate first fetch
    pollRef.current = setInterval(pollAll, 30_000);
    ...
    <main className="min-h-screen px-safe pt-[var(--space-6)] pb-[calc(var(--space-20)+env(safe-area-inset-bottom))]">
      <div className="mx-auto w-full max-w-[420px] px-4">
        ...
        <div className="mb-6 flex items-center gap-3">
          ...
          <h1 className="text-[20px] font-[var(--fw-bold)] text-app">Vuelos en directo</h1>
        </div>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center gap-3 pt-20 text-center">
            ...
            <p className="text-[15px] font-semibold text-app">Sin vuelos activos</p>
            <p className="max-w-[240px] text-[13px] text-[var(--text-secondary)]">
              Cuando un amigo suba su boarding pass y esté volando aparecerá aquí automáticamente.
            </p>
          </div>
        ) : (
          <div className="space-y-4">
...
function FlightWidget({ ticket, position }: { ticket: PlanTicket; position: FlightPosition | null }) {
  ...
  const isStale    = staleMs != null && staleMs > 90_000; // >90s sin actualizar
