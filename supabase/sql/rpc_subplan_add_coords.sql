-- ============================================================
-- Añadir coordenadas a subplan para evitar llamadas a Geocoding API
-- Ejecutar en Supabase Dashboard → SQL Editor
-- ============================================================

-- ─── 1. Columnas ──────────────────────────────────────────
ALTER TABLE subplan
  ADD COLUMN IF NOT EXISTS ubicacion_lat     float8,
  ADD COLUMN IF NOT EXISTS ubicacion_lng     float8,
  ADD COLUMN IF NOT EXISTS ubicacion_fin_lat float8,
  ADD COLUMN IF NOT EXISTS ubicacion_fin_lng float8;

-- ─── 2. fn_subplan_list (recrear con nuevos campos) ───────
...
-- ─── 3. fn_subplan_create (recrear con nuevos parámetros) ─
